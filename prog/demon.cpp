...
{
	int i, max_layer_needed, num_clones_in_deme, chosen_clone, deme_num, geno_num, index_in_deme, clone_pop;
	float old_rate;
	int old_pop;
	...
	for(deme_num = 0; deme_num < num_demes; deme_num++) {
		...
		for(index_in_deme = 0; index_in_deme < num_clones_in_deme; index_in_deme++) {
			...
			if(fabs(clones_rates_in_deme[deme_num][index_in_deme] - old_rate) > 0.05) {
	...
	*diversity = calculate_diversity(num_matrix_cols, num_cells, populations, either_matrix, dmax, rao);
	...
	if(!well_mixed) {
		...
		*edge_diversity = calculate_edge_diversity(num_matrix_cols, num_cells, either_matrix, dmax, num_demes, num_clones, clone_genotype, idum, rao, position_in_edge_list, demes_at_edge, sides_at_edge, genotype_edge_pop);
		...
		for(i=0; i<num_demes; i++) within_deme_diversity[i] = calculate_within_deme_diversity(i, num_matrix_cols, num_clones, clone_genotype, either_matrix, dmax, rao);
	...
	int **geno_or_driver_ints, float **geno_or_driver_floats, int *at_edge)
{
	...
	sampled_cells = samples * biopsy_size_per_sample;
	for(i = 0; i < num_matrix_cols; i++) genotype_populations_in_sample[i] = 0;
	for(i = 0; i < sampled_cells; i++) {
		geno_num = weighted_random_ints(geno_or_driver_ints[POPULATION], idum, 0, num_matrix_cols);
...
float calculate_diversity(int num_matrix_cols, int N, int *populations, int **either_matrix, int dmax, int rao)
{
	...
	for(i=0; i<num_matrix_cols; i++) for(j=0; j<i; j++) {
		pop1 = populations[i];
		pop2 = populations[j];

		if(rao) distance = MIN(either_matrix[j][i], dmax); // rao = 1 => use Rao's quadratic diversity
		else distance = 1; // otherwise use Simpson's index
		diversity += 2 * (float)pop1 / N * (float)pop2 / N * distance;
...
float calculate_within_deme_diversity(int deme_index, int num_matrix_cols, int num_clones, int *clone_genotype, int **either_matrix, int dmax, int rao)
{
	int i, j;
	int pop1, pop2;
	float diversity = 0;
	...
	for(i=0; i<deme_ints[NUM_CLONES_IN_DEME][deme_index]; i++) for(j=0; j<i; j++) {
		...
		if(rao) distance = MIN(either_matrix[geno_num2][geno_num1], dmax); // rao = 1 => use Rao's quadratic diversity
		else distance = 1; // otherwise use Simpson's index

		diversity += 2 * (float)pop1 / cells_in_deme * (float)pop2 / cells_in_deme * distance;
	}
